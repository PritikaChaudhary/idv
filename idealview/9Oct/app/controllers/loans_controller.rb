class LoansController < ApplicationController
 ...
 def index
 	...
 	#File.rename "#{Rails.root}/pdfs/hello.pdf", "#{Rails.root}/public/temp/hellox.pdf"
 	@bucketName = S3_BUCKET_NAME
 	#File.open(Rails.root.join('public', 'temp', 'hellox.pdf'), 'wb') do |file|
      
      contents =  File.read("#{Rails.root}/public/temp/edit.png")
      #abort("#{contents}")
      # Make an object in your bucket for your upload
      loan_id = "4100"
      key_name = loan_id+'/edit.png'
      obj = S3.put_object(
               acl: "public-read",
               bucket: @bucketName,
               key: key_name,
...
#      
  ...
  def upload_main_image
   loan=Loan.find_by_id(params[:id].to_i)
   unless loan.added_by.blank?
     user = User.find_by_id(loan.added_by)
     my_bucket = user.bucket_name
   end
