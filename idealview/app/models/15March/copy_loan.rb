class CopyLoan
  ...
  def pay_offs
    PayOff.all(:copy_loan_id => self.id.to_s)
  end
  ...
  def broker
    Broker.find_by_id(self.broker_id)
  end
  ...
  def active_user
    id = self.id
    LoanLender.find_by_copy_loan_id_and_status("#{id}", "active")
  end

  def use_funds
    UseFund.all(:copy_loan_id => "#{self.id}")
  end

  def copy_borrowers
    CopyBorrower.all(:copy_loan_id => "#{self.id}", :order => :id.desc)
  end

  def copy_collaterals
    CopyCollateral.all(:copy_loan_id => "#{self.id}", :order => :id.desc)
  end
  
  
  #grab the featured image to display on the overview page
  def featured_image
    images = Image.all(:loan_id => self.loan_id)
    images.each do |img|
  ...
  def documents
    documents = Document.all(:loan_id => self.loan_id)
  end
  ...
  def docs_by_category category
    Document.find_all_by_loan_id_and_category(self.loan_id, category)
  end
