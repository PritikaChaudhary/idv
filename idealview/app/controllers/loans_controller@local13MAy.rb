  ...
  def show
    ...
    buckets.each do |bucket|
      ...
      if bname.include? current_user.id
        have_bucket = "yes"
        @bucket_size = @bucket_size+bucket.content_length
      end
    end
  ...
  if (loan.info['added_by'].blank?)
   ...
   files.each do |file_io|
    ...
    file_new = "img_" +Time.now.strftime("%m-%d-%y_%H:%M:%S")+ext
     # file_new=file_io.original_filename
      ...
      # check = image_optim.optimize_image("public/temp/#{file_new}")
      # Make an object in your bucket for your upload
      ...
      ############# Google Cloud Storage ###########
          ...
          if !store_data.blank?
            ...
            if check && check.loan_id == loan.id
              ...
              image = Image.new(:loan_id=>loan._id,:user_id=>uId, :from=>"google", :file_id=>structure, :name=>file_new, :url=>"#{store_data.public_url}");
            end
    ...
    image_optim = ImageOptim.new(:jpegrecompress  => {:allow_lossy =>true, :quality => 4})
     ...
     image_optim.optimize_image!("#{Rails.root}/public/temp/#{file_new}")
      # abort("#{check}")
      # data = File.read("#{check}")
      # abort("#{data}")
        # file.write(contents)
        # abort("#{contents}")
        #  structure = "compress_files/#{file_new}"
        #  store_data = StorageBucket.files.new(
        #   key: "#{structure}",
        #   body: contents,
        #   public: true
