class BrokersController < ApplicationController
  ...
  def add_user
   if @infoBroker.parent_broker.blank?
        ...
        ########## Add Sub User Permission #################
        if !@infoBroker.plan.blank? 
            if @infoBroker.plan == "Business"
              @subusers=Broker.all(:delete.ne => 1, :parent_user => current_user.id.to_s)
              @num_sub = @subusers.count
              if @num_sub==5 || @num>=5
                @add_sub =  "no"
              else
                @add_sub =  "yes"
        ...
        ########## End Add Sub User Permission #################
        @user_id = current_user.id
        @user_email = current_user.email
        @sub_brokers = Broker.all(:parent_broker => @infoBroker.id.to_s)
        @num = @sub_brokers.count
    else
        @user_id = @infoBroker.parent_user
        @brokr = Broker.find_by_id(@infoBroker.parent_broker)
         ########## Add Sub User Permission #################
        if !@brokr.plan.blank?  
            if @brokr.plan == "Business"
              @brokers=Broker.all(:delete.ne => 1, :parent_broker => @infoBroker.parent_broker.to_s)
              @num_sub =  @brokers.count
              if @num_sub==5 || @num>=5
                @add_sub =  "no"
              else
                @add_sub =  "yes"
        ...
        @sub_brokers = Broker.all(:parent_broker => @infoBroker.parent_broker)
        @num = @sub_brokers.count
    end
