class LoansController < ApplicationController
 ...
 def index
  ...
  # lgroups=LenderGroup.all
  # lgroups.each do |lgroup|
  #   @checkLender = LoanLender.all(:lender_id => lgroup.lender_id)
  #   if @checkLender.blank?
      
  #     LenderGroup.delete_all(:lender_id => lgroup.lender_id)
  #   else
  #     # abort("#{@checkLender.inspect}")
  #   end
  # end

  # new_lgroups=LenderGroup.all
  # abort("#{lgroups.inspect} -- #{new_lgroups.inspect}")
  ...
  if current_user.blank?
   ...
   # abort("#{cookies[:send_fd]}")
    # @loans = Loan.all
    # @loans.each do |loan|
    #   @collaterals = Collateral.all(:loan_id => loan.id.to_i)
    #   sort = 1 
    #   @collaterals.each do |collateral|
    #     collat = Collateral.find_by_id("#{collateral.id}")
    #     collat.sort_id = sort
    #     collat.save
    #     sort = sort+1
    #   end
    ...
    # @loans = Loan.all
    # @loans.each do |loan|
    #   lInfo = Loan.find_by_id(loan.id.to_i)
    #   progress = check_progress(loan.id.to_i)
    #   # abort("#{progress}")
    #     unless lInfo.info.blank?
    #       if defined? lInfo.info['State3']
    #         unless lInfo.info['State3'].blank?
    #           lInfo.loan_state = lInfo.info['State3'].downcase 
    #         end
    #       end

    #       if defined? lInfo.info['_LendingCategory']
    #         unless lInfo.info['_LendingCategory'].blank?
    #           if lInfo.info['_LendingCategory'] == "Private Real Estate Loan"
    #             if defined? lInfo.info['_LendingTypes']
    #               unless lInfo.info['_LendingTypes'].blank?
    #                 lInfo.lending_type = lInfo.info['_LendingTypes']     
    #               end
    #             end
    #           elsif lInfo.info['_LendingCategory'] == "Business Financing"
    #             if defined? lInfo.info['_BusinessFinancingTypes']
    #               unless lInfo.info['_BusinessFinancingTypes'].blank?
    #                 lInfo.lending_type = lInfo.info['_BusinessFinancingTypes']     
    #               end
    #             end
    #           elsif lInfo.info['_LendingCategory'] == "Equity and Crowdfunding"
    #             if defined? lInfo.info['_EquityandCrowdFunding']
    #               unless lInfo.info['_EquityandCrowdFunding'].blank?
    #                 lInfo.lending_type = lInfo.info['_EquityandCrowdFunding']     
    #               end
    #             end
    #           elsif lInfo.info['_LendingCategory'] == "Residential or Commercial Mortgage"
    #             if defined? lInfo.info['_MortageTypes']
    #               unless lInfo.info['_MortageTypes'].blank?
    #                 lInfo.lending_type = lInfo.info['_MortageTypes']     
    #               end
    #             end
    #           end
    #         end
    ...
    #       if defined? lInfo.info['transaction_type']
    #         unless lInfo.info['transaction_type'].blank?
    #           lInfo.transaction_type = lInfo.info['transaction_type']
    #         end
    #       end

    #       if defined? lInfo.info['_EstimatedMarketValue']
    #         unless lInfo.info['_EstimatedMarketValue'].blank?
    #           lInfo.estimated_value = lInfo.info['_EstimatedMarketValue'].to_i
    #         end
    #       end
    ...
    # @credit_key = CreditKey.last
    # abort("#{@credit_key.inspect}")
   #################### Create Broker  ######################
   #  @loans = Loan.all
   #  @emails = Array.new
   #  binfo = Array.new
   ...
   #  end
    ...
    if @checkAdmin!=true && @checkBroker==true
      ...
      @loans = Loan.paginate(:email =>@broker_emails, :delete.ne => 1, :delete_broker.ne => 1, :archived.ne => true, :per_page => 50, :page => params[:page], :total_entries => @total_loans, :order => :id.desc) 
      # abort("#{@loans.inspect}")
    else
      ...
      @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne => 1, :archived.ne => true, :per_page => 50, :page => params[:page], :total_entries => @total_loans, :order => :id.desc)
      # @loans = Loan.all
  ...
  def delete_borrower
    related_to = Borrower.find_by_id("#{params[:id]}")
    Borrower.delete_all(:id => params[:id])
    @borrowers = Borrower.all(:loan_id => related_to.loan_id)

    @borrowers.each do |borrower|
      ############### Previous sorting ############

        borwr = Borrower.find_by_id("#{borrower.id}")
        borwr.previous_sort_id = borwr.sort_id
        borwr.save

      ############## End PRevious Sorting ###########
    end
    render text: "done"
  end

  def hide_copy_borrower
  ...
  def delete_copy_borrower
    related_to = CopyBorrower.find_by_id("#{params[:id]}")
    CopyBorrower.delete_all(:id => params[:id])
    @borrowers = CopyBorrower.all(:copy_loan_id => related_to.copy_loan_id)
  ...
  def delete_collateral
    related_to = Collateral.find_by_id("#{params[:id]}")
    Collateral.delete_all(:id => params[:id])
    @collaterals = Collateral.all(:loan_id => related_to.loan_id)
    @collaterals.each do |collateral|
      update_collat = Collateral.find_by_id("#{collateral.id}")
      update_collat.previous_sort_id = update_collat.sort_id
      update_collat.save
    end
  ...
  def delete_copy_collateral
    related_to = CopyCollateral.find_by_id("#{params[:id]}")
    CopyCollateral.delete_all(:id => params[:id])
    @collaterals = CopyCollateral.all(:copy_loan_id => related_to.copy_loan_id)
    @collaterals.each do |collateral|
      update_collat = CopyCollateral.find_by_id("#{collateral.id}")
      update_collat.previous_sort_id = update_collat.sort_id
      update_collat.save
    end
  ...
  def hide_copy_collateral
    collateral = CopyCollateral.find_by_id(params[:id])
    # abort("#{borrower.inspect}")
    collateral.hide = 1
    collateral.save
  ...
  def show
    ...
    # @img = Image.find_by_id("5837e1dd52d08a12bb00000e")

    # @images = Image.all(:loan_id => 5157)
    # abort("#{@img.inspect} -- #{@images.inspect}")
    ...
    # @loans = Loan.all(:loan_state.ne => "")
    # abort("#{@loans.inspect}")

    @groups = ShopGroup.all

    # @loan_form_link = "https://dash.idvstage.us/loans/edit_loan/#{loanInfo.id}"
    @edit_email_link = Email.find_by_name("Form Link")
    # abort("#{@edit_email_link.inspect}")
  ...
  #  bucket = S3.create_bucket(bucket: 'testbusketxyz')
    ...
    if @loan.created_by_info == "Embed Form"
      @new_custom_fields = NewCustomField.all(:user_id => @loan.enterprise_id)
      @new_custom_tab = Custom.find_by_form_and_user_id("Custom Fields", "#{@loan.enterprise_id}")
    else
   ...
   # abort("#{@loan.inspect}")
   @borrowers = Borrower.all(:loan_id => @loan.id, :order => :sort_id.asc)
 ...
 # abort("#{noi_ytd} #{noi_ytd_arr.inspect}")
  ...
  def upload_main_image
   ...
   file.write(contents)
    ...
    progress = check_progress(params[:id].to_i)
  end
  ...
  def upload_main_imagex
   ...
   if urll.include? "data:image/jpeg;base64,"
      imgxPath = urll.gsub("data:image/jpeg;base64,", '')
   elsif urll.include? "data:image/jpg;base64,"
      imgxPath = urll.gsub("data:image/jpeg;base64,", '')
   elsif urll.include? "data:image/png;base64,"
      imgxPath = urll.gsub("data:image/png;base64,", '')
   elsif urll.include? "data:image/gif;base64,"
      imgxPath = urll.gsub("data:image/png;base64,", '')
   elsif urll.include? "data:image/bmp;base64,"
      imgxPath = urll.gsub("data:image/png;base64,", '')
   end
   ...
   file.write img_path
      ...
      ############### Upload To Dropbox ##########################
        ...
        unless current_user.blank?
          ...
          unless current_user.dropbox.blank?
            if @adminLogin == true && current_user.dropbox.not_sync != 1
              @drop_upload = 1
            end
            ...
            if plan == "enterprise" && current_user.dropbox.not_sync != 1
              @drop_upload = 1
            end
          end 

          unless current_user.dropbox.blank?
            @loan_data=Loan.find_by_id(params[:id].to_i)
            if @drop_upload == 1 &&  @loan_data.not_sync != 1
              headers = {"Authorization" => "Bearer #{current_user.dropbox.token}", "Content-Type" => "application/json" }
              ...
              data = {"path" => "/IDV/#{@loan_data.name}/images/#{file_name}", "url" => "#{store_data.public_url}"}
              jdata = data.to_json
      ...
      ############### End Upload To Dropbox ######################
       ...
       #clear all featured image tags
        ...
        # if obj.last_page?
            ...
            image.top = params[:top].to_s
            image.width = params[:width].to_s
            image.left = params[:left].to_s
            image.featured = true
    ...
    unless current_user.blank?
        ...
        unless @infoBroker.blank?
          unless @infoBroker.plan.blank?
            @plan = @infoBroker.plan.downcase
          else
            @plan = ""
          end
        else
        ...
        if !@infoBroker.blank? && @infoBroker.plan=="free"
          ...
          # abort("#{@fileUpload}")
        elsif !@infoBroker.blank? &&  @plan == "business"
        ...
        elsif !@infoBroker.blank? &&  @plan == "enterprise"
        ...
        unless current_user.blank?
          ...
          unless current_user.dropbox.blank?
            ...
            if @drop_upload == 1  &&  @loan_data.not_sync != 1
              ...
              # response = http.request(request)
              # response.set_form_data(uri.path, {"path" => "/IDV/abc/images/090816005417_images.jpeg", "url" => "https://idealview.storage.googleapis.com/571721054749e85c11000002/5106/090816005417_images.jpeg"}, headers)
              data = {"path" => "/IDV/#{@loan_data.name}/images/#{file_new}", "url" => "#{store_data.public_url}"}
              jdata = data.to_json
              response = http.post(uri.path, jdata, headers)
              rsp = response.body
            end
      ...
      unless current_user.blank?
        ...
        unless current_user.dropbox.blank?
          ...
          if @drop_upload == 1 &&  @loan_data.not_sync != 1
            ...
            # response = http.request(request)
            # response.set_form_data(uri.path, {"path" => "/IDV/abc/images/090816005417_images.jpeg", "url" => "https://idealview.storage.googleapis.com/571721054749e85c11000002/5106/090816005417_images.jpeg"}, headers)
            data = {"path" => "/IDV/#{@loan_data.name}/documents/#{new_filename}", "url" => "#{store_data.public_url}"}
            jdata = data.to_json
            response = http.post(uri.path, jdata, headers)
            rsp = response.body
          end
 ...
 def recent actn=nil
    ...
    if @checkAdmin!=true && @checkBroker==true
      ...
      @total_loans = Loan.count(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true)
       if params[:field_name] == "name"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_name.asc)
          else
            @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_name.desc)
          end
       elsif params[:field_name] == "amount"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :net_loan_amount.asc)
          else
            @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :net_loan_amount.desc)
          end

       elsif params[:field_name] == "location"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_location.asc)
          else
            @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_location.desc)
          end
       elsif params[:field_name] == "progress"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :progress.asc)
          else
            @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :progress.desc)
          end
       else
          @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :id.desc)
       end
      ...
      #@loans = Loan.paginate(:delete.ne => 1, :archived.ne => true, :order => :id.desc, :per_page => 10, :page => params[:page])
       ...
       if params[:field_name] == "name"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_name.asc)
          else
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_name.desc)
          end
       elsif params[:field_name] == "amount"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :net_loan_amount.asc)
          else
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :net_loan_amount.desc)
          end

       elsif params[:field_name] == "location"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_location.asc)
          else
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_location.desc)
          end
       elsif params[:field_name] == "progress"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :progress.asc)
          else
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :progress.desc)
          end
       else
          @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :id.desc)
       end
  ...
  def priority actn=nil
    ...
    if @checkAdmin!=true && @checkBroker==true
      ...
      if params[:field_name] == "name"
        if params[:sort_type] == "asc"
          @loans = Loan.paginate(:on_priority=>1, :email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_name.asc) 
        else
          @loans = Loan.paginate(:on_priority=>1, :email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_name.desc) 
        end
      elsif params[:field_name] == "amount"
        if params[:sort_type] == "asc"
          @loans = Loan.paginate(:on_priority=>1, :email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :net_loan_amount.asc) 
        else
          @loans = Loan.paginate(:on_priority=>1, :email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :net_loan_amount.desc) 
        end

      elsif params[:field_name] == "location"
        if params[:sort_type] == "asc"
          @loans = Loan.paginate(:on_priority=>1, :email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_location.asc) 
        else
          @loans = Loan.paginate(:on_priority=>1, :email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_location.desc) 
        end
      elsif params[:field_name] == "progress"
        if params[:sort_type] == "asc"
          @loans = Loan.paginate(:on_priority=>1, :email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :progress.asc) 
        else
          @loans = Loan.paginate(:on_priority=>1, :email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived.ne => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :progress.desc) 
        end
      else
      ...
      if params[:field_name] == "name"
        if params[:sort_type] == "asc"
          @loans = Loan.paginate(:on_priority=>1, :delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_name.asc)
        else
          @loans = Loan.paginate(:on_priority=>1, :delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_name.desc)
        end
      elsif params[:field_name] == "amount"
        if params[:sort_type] == "asc"
          @loans = Loan.paginate(:on_priority=>1, :delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :net_loan_amount.asc)
        else
          @loans = Loan.paginate(:on_priority=>1, :delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :net_loan_amount.desc)
        end

      elsif params[:field_name] == "location"
        if params[:sort_type] == "asc"
          @loans = Loan.paginate(:on_priority=>1, :delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_location.asc)
        else
          @loans = Loan.paginate(:on_priority=>1, :delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_location.desc)
        end

      elsif params[:field_name] == "progress"
        if params[:sort_type] == "asc"
          @loans = Loan.paginate(:on_priority=>1, :delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :progress.asc)
        else
          @loans = Loan.paginate(:on_priority=>1, :delete.ne => 1, :delete_admin.ne =>1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :progress.desc)
        end
      else
  ...
  def archived_loans actn=nil
      #################### Check Login #######################
      @select_type = "archived"
      roles=current_user.roles
      @names = Array.new
      roles.each do |role|
        @names << role.name
      ...
      if @checkAdmin!=true && @checkBroker==true
        ...
        @total_loans = Loan.count(:delete_broker.ne =>1, :email =>@broker_emails, :delete.ne => 1, :archived => true)
        
        
        if params[:field_name] == "name"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_name.asc) 
          else
            @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_name.desc) 
          end
        elsif params[:field_name] == "amount"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :net_loan_amount.asc)
          else
           @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :net_loan_amount.desc) 
          end

        elsif params[:field_name] == "location"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_location.asc)
          else
            @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_location.desc)
          end
        elsif params[:field_name] == "progress"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order =>  :progress.asc)
          else
            @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order =>  :progress.desc) 
          end
        else
           @loans = Loan.paginate(:email =>@broker_emails, :delete_broker.ne =>1, :delete.ne => 1, :archived => true , :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order =>  :id.desc)
        end

      else
        @total_loans = Loan.count(:delete.ne => 1, :delete_admin.ne =>1, :archived => true)
        
        if params[:field_name] == "name"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_name.asc)
          else
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_name.desc)
          end
        elsif params[:field_name] == "amount"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :net_loan_amount.asc)
          else
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :net_loan_amount.desc)
          end

        elsif params[:field_name] == "location"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_location.asc)
          else
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_location.desc)
          end

        elsif params[:field_name] == "progress"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :progress.asc)
          else
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :progress.desc)
          end
        else
            @loans = Loan.paginate(:delete.ne => 1, :delete_admin.ne =>1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :id.desc)
        end
      end
      ...
      # abort("#{@loans.inspect}")
       if actn == "delete"
        ...
        flash.now[:notice] = "Sorting by archived loans"
       end
       render partial: 'loans/all_loans'
  end
...
def generate_html
  ...
  File.open(save_path, 'wb') do |file|
   ...
   pdfInfo.link_type = params[:link_type]
   pdfInfo.save
...
def send_to_ids
  ...
  all_ids.each do |id|
    ...
    unless @funds.blank?
     ...
     @loan_images =  Image.where(:loan_id => id.to_i).fields(:name, :url).all

    myHash['images'] = Array.new
    
    unless @loan_images.blank?
      @loan_images.each do |limg|
        image_urls = Hash.new
        myHash['images']<< limg.url
      end
    end
    ...
    myHash['sender_email'] = "#{current_user.email}"
    myHash['receiver_username'] = "#{current_user.cpc_subdomain}"
    ...
    if server_host.include? "idealview"
      send_url = "https://#{current_user.cpc_subdomain}.idealsuite.com/index.php?r=loginapi/getidvloan"
      # abort("#{send_url}")
      uri = URI.parse("https://#{current_user.cpc_subdomain}.idealsuite.com/index.php?r=loginapi/getidvloan");
      ...
      check_url = "http://#{current_user.cpc_subdomain}.xlr8seo.com/index.php?r=loginapi/getidvloan"
  ...
  # abort("#{@rsp.inspect}")
  @ids_rsp = JSON.parse(@rsp)
  if @ids_rsp['status'] == "false"
    render text: "false"
  else
    render text: "done"
  end
...
################################ Send Loan Info ########################################
 ...
 def send_loan_info
    ...
    # abort("#{jdata.inspect}#{jfiles.inspect}")
     ...
     data = Hash.new
    data['files'] = jfiles
    data['loan_info'] = jdata

    loan_data = data.to_json
    ...
    if current_user.blank?
      ...
      if @adminLogin != true
        if !@infoBroker.blank? 
          ...
          unless current_user.max_storage.blank?
            munit = current_user.max_storage.gsub(/\d+/,'')
            mint = current_user.max_storage.to_i
          end
...
def delete_broker_loans
 ...
 def save_loc
  ...
  loc = "#{params[:City3]}, #{params[:State3]}"
  @loan.sort_location = loc.downcase 
  st = "#{params[:State3]}"
  @loan.loan_state = st.downcase 
...
def pdforder
  ...
  def custom_search
   ...
   if params['sorting'] == "recent"
      ...
      if ids.blank?
        ...
        if params[:field_name] == "name"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_name.asc)
          else
            @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_name.desc)
          end
        elsif params[:field_name] == "amount"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :net_loan_amount.asc)
            else
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :net_loan_amount.desc)            
            end

        elsif params[:field_name] == "location"

            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_location.asc)
            else
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_location.desc)            
            end
        elsif params[:field_name] == "progress"

            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :progress.asc)
            else
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :progress.desc)            
            end
        else
  ...
  ################ Incomplete Loan ###################
    ...
    if ids.blank?
        ...
        if params[:field_name] == "name"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:id=>ids, :incomplete=>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_name.asc)
          else
            @loans = Loan.paginate(:id=>ids, :incomplete=>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_name.desc)
          end
        elsif params[:field_name] == "amount"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>ids, :incomplete=>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :net_loan_amount.asc)
            else
              @loans = Loan.paginate(:id=>ids, :incomplete=>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :net_loan_amount.desc)
            end

        elsif params[:field_name] == "location"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>ids, :incomplete=>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_location.asc)
            else
              @loans = Loan.paginate(:id=>ids, :incomplete=>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_location.desc)
            end

        elsif params[:field_name] == "progress"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>ids, :incomplete=>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :progress.asc)
            else
              @loans = Loan.paginate(:id=>ids, :incomplete=>1, :delete.ne => 1, :archived.ne => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :progress.desc)
            end
   ...
   elsif  params['sorting'] == "archived_loan"
      if ids.blank?
        @loans = Array.new
        @empty =  "No Loan Found."
      else
        # @loans = Loan.all(:id=>ids, :on_priority=>1, :order => :priority_num.desc)
        @total_loans = Loan.count(:id=>ids, :delete.ne => 1, :archived => true);

        if params[:field_name] == "name"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_name.asc)
          else
            @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_name.desc)
          end
        elsif params[:field_name] == "amount"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :net_loan_amount.asc)
            else
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :net_loan_amount.desc)
            end

        elsif params[:field_name] == "location"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_location.asc)
            else
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_location.desc)
            end

        elsif params[:field_name] == "progress"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :progress.asc)
            else
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :progress.desc)
            end
        else
          @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived => true, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :priority_num.asc)
        end
      ...
      flash.now[:notice] = "Searching by archived loans."
      render partial: 'loans/sort_loans'
  ...
  ################ Incomplete Loan ###################
      ...
      if ids.blank?
        ...
        if params[:field_name] == "name"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_name.asc)
          else
            @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_name.desc)
          end
        elsif params[:field_name] == "amount"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :net_loan_amount.asc)
            else
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :net_loan_amount.desc)
            end

        elsif params[:field_name] == "location"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_location.asc)
            else
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_location.desc)
            end

        elsif params[:field_name] == "progress"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :progress.asc)
            else
              @loans = Loan.paginate(:id=>ids, :delete.ne => 1, :archived.ne => true, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :progress.desc)
            end
        else
...
#fdgdfgdfgfdg
    ...
    if params[:stepping] == "2"
      ...
      if params[:nvid].blank?
        ...
        if params[:stepping] == "1" 
          ...
          dbloan.info['gross_loan_amount'] = params['_NetLoanAmountRequested0']
          dbloan.net_loan_amount = params['_NetLoanAmountRequested0'].to_i
          unless params['purchase_cash_contribute'].blank?
            dbloan.info['acash_to_contribute'] = params['purchase_cash_contribute']
          end 
          unless params['purchase_price'].blank?
      ...
      unless params[:_LoanName].blank?
        dbloan.name= params[:_LoanName]
        dbloan.sort_name= params[:_LoanName].downcase
      end
      if params[:stepping]
     ...
     unless dbloan.blank?
      progress= check_progress(dbloan.id.to_i)
      # abort("#{progress.inspect}")
     end
     # render text: "fgdfg"
     render json: dbloan
  end
  ...
  #  checks whether the request comes from a trusted source
   ...
   def detail
    ...
    if !loan_url.blank?
       ...
       ############## Bar Chart ###################
        @funds = UseOfFund.all(:loan_id => @loan.id, :amount.ne => 0)
        @amnt = ""
        unless @funds.blank?
            @uses = Array.new
            @amounts = Array.new
       ...
       @borrowers = Borrower.all(:loan_id => @loan.id, :hide.ne=>1, :order => :sort_id.asc)
       ...
       @files = FolderFile.all(:loan_id => @loan.id)
        ...
        #################### Analysis ############################
        @documents = Document.all(:loan_id => @loan.id, :delete.ne => 1)
        unless @loan.hide_fields.blank? 
       ...
       ############## Bar Chart ###################
        ...
        @funds = UseOfFund.all(:loan_id => @loan.id, :amount.ne =>"")
        # abort("#{@funds.inspect}")
        @amnt = ""
        unless @funds.blank?
            @uses = Array.new
    ...
    id = params[:id]
      ...
      unless borrowers.blank?
        borrowers.each do |borrower|
          ...
          brwr = CopyBorrower.new
          brwr.copy_loan_id = "#{@copyLoanId}"
          @brwrs = borrower
          @brwrs.keys.each do |key|
            keyz = key.to_a
            if !keyz[0].include? "_id"
              brwr[keyz[0]] = borrower[keyz[0]]
            end
          end
          brwr.save
      ...
      unless collaterals.blank?
        collaterals.each do |collateral|
          coltrl = CopyCollateral.new
          coltrl.copy_loan_id = "#{@copyLoanId}"
          @cltrls = collateral
          @cltrls.keys.each do |key|
            keyc = key.to_a
            if !keyc[0].include? "_id"
              coltrl[keyc[0]] = collateral[keyc[0]]
            end
          end
  ...
  def shop_loans
    ...
    @loans = CopyLoan.all(:sort => :id.desc)
    ...
    @loans.each do |loan|
      ...
      # @borrowers = CopyBorrower.all(:copy_loan_id =>"#{loanInfo.id}")
      # unless @borrowers.blank?
      #   @i=1
      #   @borrowers.each do |borrower|
      #     brwr = CopyBorrower.find_by_id("#{borrower.id}")
      #     brwr.sort_id = @i
      #     brwr.save
      #     @i = @i+1
      #   end
      ...
      @collaterals = CopyCollateral.all(:copy_loan_id =>"#{loanInfo.id}")
      unless @collaterals.blank?
        @i=1
        @collaterals.each do |collateral|
          colat = CopyCollateral.find_by_id("#{collateral.id}")
          colat.sort_id = @i
          colat.save
          @i = @i+1
        end
  ...
  def shop_loan
   ...
   @borrowers = CopyBorrower.all(:copy_loan_id => @loan.id.to_s,:sort => :sort_id.asc)
     # abort("#{@borrowers.inspect}")
    @brwrId = Array.new
    @brwrId << "loan_highlight"
    @brwrId << "loc_location"
 ...
 # abort("#{noi_ytd} #{noi_ytd_arr.inspect}")
  ...
  def add_loan_funds
    ...
    progress = check_progress(params[:loan_id].to_i)
    # abort("#{check.inspect}")
    render text: 'done'
  end
  ...
  def search
    ...
    if params['sorting_type'] == "recent"
        ...
        if @ids.blank?
         ...
         if params[:field_name] == "name"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>@ids, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_name.asc)
            else
              @loans = Loan.paginate(:id=>@ids, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_name.desc)
            end
         elsif params[:field_name] == "amount"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>@ids, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :net_loan_amount.asc)
            else
              @loans = Loan.paginate(:id=>@ids, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :net_loan_amount.desc)
            end

         elsif params[:field_name] == "location"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>@ids, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_location.asc)
            else
              @loans = Loan.paginate(:id=>@ids, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_location.desc)
            end
         elsif params[:field_name] == "progress"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>@ids, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :progress.asc)
            else
              @loans = Loan.paginate(:id=>@ids, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :progress.desc)
            end
         else
            @loans = Loan.paginate(:id=>@ids, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :id.desc)
         end
      ...
      if @ids.blank?
        ...
        if params[:field_name] == "name"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:id=>@ids, :incomplete=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_name.asc)
          else
            @loans = Loan.paginate(:id=>@ids, :incomplete=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_name.desc)
          end
        elsif params[:field_name] == "amount"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>@ids, :incomplete=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :net_loan_amount.asc)
            else
              @loans = Loan.paginate(:id=>@ids, :incomplete=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :net_loan_amount.desc)
            end

        elsif params[:field_name] == "location"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>@ids, :incomplete=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_location.asc)
            else
              @loans = Loan.paginate(:id=>@ids, :incomplete=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :sort_location.desc)
            end
        elsif params[:field_name] == "progress"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>@ids, :incomplete=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :progress.asc)
            else
              @loans = Loan.paginate(:id=>@ids, :incomplete=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :progress.desc)
            end
        else
            @loans = Loan.paginate(:id=>@ids, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans , :order => :id.desc)
        end
      ...
      if @ids.blank?
        ...
        if params[:field_name] == "name"
          if params[:sort_type] == "asc"
            @loans = Loan.paginate(:id=>@ids, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_name.asc)
          else
            @loans = Loan.paginate(:id=>@ids, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_name.desc)
          end
        elsif params[:field_name] == "amount"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>@ids, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :net_loan_amount.asc)
            else
              @loans = Loan.paginate(:id=>@ids, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :net_loan_amount.desc)
            end

        elsif params[:field_name] == "location"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>@ids, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_location.asc)
            else
              @loans = Loan.paginate(:id=>@ids, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :sort_location.desc)
            end
        elsif params[:field_name] == "progress"
            if params[:sort_type] == "asc"
              @loans = Loan.paginate(:id=>@ids, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :progress.asc)
            else
              @loans = Loan.paginate(:id=>@ids, :on_priority=>1, :per_page => params[:records], :page => params[:page], :total_entries => @total_loans, :order => :progress.desc)
            end
        else
  ...
  def save_edit_pic
    ...
    image.top = params[:top]
    image.left = params[:left]
  ...
  def add_new_borrower
    ...
    num_of_borrower = Borrower.count(:loan_id => params[:loan_id].to_i)
    borrower = Borrower.new
    borrower.loan_id = params[:loan_id].to_i
    borrower.sort_id = num_of_borrower + 1
    borrower.save

     @loan = Loan.find_by_id(params[:loan_id].to_i)

    @borrowers = Borrower.all(:loan_id => params[:loan_id].to_i, :order => :sort_id.asc)
    ...
    @type= ""
    @borrowers = Borrower.all(:loan_id => params[:loan_id].to_i, :order => :sort_id.asc)
    @num_of_borrowers = Borrower.count(:loan_id=>params[:loan_id].to_i)
    render partial: 'next_borrower'
  end

  def sort_borrower
    @num_of_borrowers = Borrower.count(:loan_id=>params[:loan_id].to_i)
    @borrowers = Borrower.all(:loan_id=>params[:loan_id].to_i)
   # abort("#{@borrowers.inspect}")
   # sort = 1
   # @borrowers.each do |borrower|
   #  borwr = Borrower.find_by_id("#{borrower.id}")
   ...
   # abort("#{@borrowers.inspect}") 
   orderarr= Array.new
    if params[:sort_type] == "up"
      @borrowers.each do |borrower|
        borwr = Borrower.find_by_id("#{borrower.id}")
        orderarr << borwr.sort_id
        if borrower.sort_id != 1
          borwr.sort_id = borrower.sort_id - 1
        else
          borwr.sort_id = @num_of_borrowers
        end
        
        borwr.save
      end
      @type = "up"
    elsif params[:sort_type] == "down"
      # abort("#{params[:sort_type]}")
      @borrowers.each do |borrower|
        borwr = Borrower.find_by_id("#{borrower.id}")
        if borrower.sort_id != @num_of_borrowers
          borwr.sort_id = borrower.sort_id + 1
        else
          borwr.sort_id = 1
        end
    ...
    # abort("#{orderarr.inspect}")
    render partial: 'next_borrower'

  end


  def sort_copyborrower
    @num_of_borrowers = CopyBorrower.count(:copy_loan_id=>params[:loan_id])
    @borrowers = CopyBorrower.all(:copy_loan_id=>params[:loan_id])
   # abort("#{@borrowers.inspect}")
   # sort = 1
   # @borrowers.each do |borrower|
   #  borwr = Borrower.find_by_id("#{borrower.id}")
   ...
   orderarr= Array.new
    ...
    @borrowers = CopyBorrower.all(:copy_loan_id => params[:loan_id], :order => :sort_id.asc)
  ...
  def sort_collateral
    @num_of_collaterals = Collateral.count(:loan_id=>params[:loan_id].to_i)
   ...
   # abort("#{@borrowers.inspect}")
    if params[:sort_type] == "up"
      ...
      @collaterals.each do |collateral|
        collat = Collateral.find_by_id("#{collateral.id}")
         # abort("#{borrower.sort_id}")
        if collateral.sort_id != 1
          collat.sort_id = collateral.sort_id - 1
        else
          collat.sort_id = @num_of_collaterals
        end
        collat.save
      end
      @type="up"
      ...
      @collaterals.each do |collateral|
        collat = Collateral.find_by_id("#{collateral.id}")
        if collateral.sort_id != @num_of_collaterals
          collat.sort_id = collateral.sort_id + 1
        else
          collat.sort_id = 1
        end
  ...
  def sort_copy_collateral
    @num_of_collaterals = CopyCollateral.count(:copy_loan_id=>params[:loan_id])
    @collaterals = CopyCollateral.all(:copy_loan_id=>params[:loan_id])
   
    if params[:sort_type] == "up"
      # abort("#{@borrowers.inspect}")
      @collaterals.each do |collateral|
        collat = CopyCollateral.find_by_id("#{collateral.id}")
         # abort("#{borrower.sort_id}")
        if collateral.sort_id != 1
          collat.sort_id = collateral.sort_id - 1
        else
    ...
    @collaterals = CopyCollateral.all(:copy_loan_id => params[:loan_id], :order => :sort_id.asc)
  ...
  def embed_add_new_borrower
    ...
    @contact_tab = Custom.find_by_form_and_user_id("Contact Information", "#{params[:id]}")
    @highlight_tab = Custom.find_by_form_and_user_id("Highlights", "#{params[:id]}")
    @summary_tab = Custom.find_by_form_and_user_id("Summary", "#{params[:id]}")
    @transaction_information_tab = Custom.find_by_form_and_user_id("Transaction Information", "#{params[:id]}")
    @use_of_funds_tab = Custom.find_by_form_and_user_id("Use Of Funds", "#{params[:id]}")
    @exit_strategy_tab = Custom.find_by_form_and_user_id("Exit Strategy", "#{params[:id]}")
    @borrowers_tab = Custom.find_by_form_and_user_id("Borrower", "#{params[:id]}")
    @collateral_tab = Custom.find_by_form_and_user_id("Collateral", "#{params[:id]}")
    @picture_tab = Custom.find_by_form_and_user_id("Picture", "#{params[:id]}")
    @document_tab = Custom.find_by_form_and_user_id("Document", "#{params[:id]}")

    render partial: 'next_embed_borrower'
  end
  ...
  def add_new_collateral
    ...
    collateral = Collateral.new
    collateral.loan_id = params[:loan_id].to_i
    collateral.sort_id = @num_of_collateral + 1
  ...
  def embed_add_new_collateral
    ...
    @summary_tab = Custom.find_by_form_and_user_id("Summary", "#{params[:id]}")
    @transaction_information_tab = Custom.find_by_form_and_user_id("Transaction Information", "#{params[:id]}")
    @use_of_funds_tab = Custom.find_by_form_iand_user_id("Use Of Funds", "#{params[:id]}")
    @exit_strategy_tab = Custom.find_by_form_and_user_id("Exit Strategy", "#{params[:id]}")
    @borrowers_tab = Custom.find_by_form_and_user_id("Borrower", "#{params[:id]}")
    @collateral_tab = Custom.find_by_form_and_user_id("Collateral", "#{params[:id]}")
    @picture_tab = Custom.find_by_form_and_user_id("Picture", "#{params[:id]}")
    ...
    render partial: 'next_embed_collateral'
  end
  ...
  def add_copy_borrower
    num = CopyBorrower.count(:copy_loan_id => params[:loan_id])
    @serial = num+1 
  ...
  def add_copyrelated_fund
    # colInfo = Collateral.find_by_id(params[:collateral_id])
    @colid = params[:collateral_id]
    @loanId = params[:loan_id]
    render partial: 'add_copyrelated_fund'
  end
  ...
  def add_copyfund_related
    ...
    collat = CopyCollateral.find_by_id(params[:collateral_id].to_s)
    relate_funds = collat.related_funds
    if relate_funds!=""
      collat.related_funds = "#{relate_funds},#{fund.id}"
    else
    ...
    render partial: 'copyrelated_funds'  
  end
    ...
    if params[:ftype] == "money"
      params.each do |key, value| 
       ...
       params.each do |key, value| 
          ...
          if key == "_LoanName"
              @loan.name = @field_val
              @loan.sort_name= @field_val.downcase
          end
          if key == "State3"
            @loan.loan_state = @field_val.downcase
          end
       end
    end 
  ...
  def borrower_changeorder
    loan_id = params[:loan_id]
    i=1
    colIds = params[:moredata].split(",").map { |s| s }
    colIds.each do |colid|
      id = colid.gsub("borrow_","")
      borInfo = Borrower.find_by_id(id)
      borInfo.sort_id = i
      borInfo.save
      i += 1
    end
  ...
  def save_loan_field
    loanInfo = Loan.find_by_id(params[:loan_id].to_i)
    if params[:field] == "_LoanSummaryWhatareyoulookingfor"
      decoded_val = URI.unescape("#{params[:value]}")
      loanInfo.info[params[:field]] = "#{decoded_val}"
    elsif params[:field] == "_ExitStrategyHowwillyoupaytheloanoff"
      decoded_val = URI.unescape("#{params[:value]}")
      loanInfo.info[params[:field]] = "#{decoded_val}"
    else
    ...
    if params[:field] == "City3"
      loc = "#{params[:value]}, #{loanInfo.info['State3']}"
      loanInfo.sort_location = loc.downcase 
    end

    if params[:field] == "State3"
      loc = "#{loanInfo.info['City3']}, #{params[:value]}"
      loanInfo.sort_location = loc.downcase 
      st = "#{params[:value]}"
      loanInfo.loan_state = st.downcase
    end
    
    if params[:field] == "_LendingTypes"
      loanInfo.lending_type = "#{params[:value]}"
    end

    if params[:field] == "_BusinessFinancingTypes"
      loanInfo.lending_type = "#{params[:value]}"
    end

    if params[:field] == "_EquityandCrowdFunding"
      loanInfo.lending_type = "#{params[:value]}"
    end

    if params[:field] == "_MortageTypes"
      loanInfo.lending_type = "#{params[:value]}"
    end

    if params[:field] == "transaction_type"
      loanInfo.transaction_type = "#{params[:value]}"
    end
  ...
  def save_loan_money
      ...
      amount = amnt.gsub(',','')
      if params[:field] == "_NetLoanAmountRequested0"
        loanInfo.net_loan_amount = amount.to_i
      end

      if params[:field] == "_EstimatedMarketValue"
        loanInfo.estimated_value = amount.to_i
      end
  ...
  def save_copyloan_money
      ...
      loanInfo.info[params[:field]] = amount
      loanInfo.save
      # progress = check_copy_progress(params[:loan_id].to_i)
      # abort("#{loanInfo.inspect}")
  ...
  def save_copycollateral_money
    colInfo = CopyCollateral.find_by_id(params['collateral_id'])
     ...
     ################# Analysis Code #############################
      ...
      loan = CopyLoan.find_by_id(colInfo.copy_loan_id)
      ...
      # if acre_table == 1
          ...
          all_acres = CopyCollateralAcres.all(:copy_loan_id => loan.id)
          total_acres = 0
          unless all_acres.blank?
        ...
        # end
         ...
         # if footage_table == 1
            all_footages = CopyCollateralSqfootage.all(:loan_id => loan.id)
            total_footages = 0
            unless all_footages.blank?
              all_footages.each do |tfootage|
                total_footages += tfootage.value.to_f
        ...
        # if gross_table == 1
          all_grosses = CopyCollateralGross.all(:loan_id => loan.id)
          total_grosses = 0
          unless all_grosses.blank?
            all_grosses.each do |tgross|
              total_grosses += tgross.value.to_f
        ...
        # if noi_table == 1
          all_nois = CopyCollateralNoi.all(:loan_id => loan.id)
          unless all_nois.blank?
            total_nois = 0
            all_nois.each do |tnoi|
              total_nois += tnoi.value.to_f
  ...
  def estimated_market_value
    ...
    loan.estimated_value = mkt_value.to_i
    loan.save
  ...
  def copy_estimated_market_value
    loanId = params[:loan_id]
    collaterals = CopyCollateral.all(:copy_loan_id =>loanId)
    mkt_value = 0
    collaterals.each do |collateral|
      unless collateral.estimated_value.blank?
        mkt_value += collateral.estimated_value.to_i
    ...
    loan = CopyLoan.find_by_id(loanId)
    loan.info['_EstimatedMarketValue'] = mkt_value
    loan.save
  ...
  def save_borrower_field
    ...
    if params[:field] == "bio"
      decoded_val = URI.unescape("#{params[:value]}")
      borInfo["#{params[:field]}"] = "#{decoded_val}"
    else
  ...
  def send_form_link
    ...
    all_email.each do |email|
      ...
      LoanUrlMailer.form_link(params[:subject], params[:msg], "#{ready_email}").deliver
    end
 ...
 # abort("#{noi_ytd} #{noi_ytd_arr.inspect}")
  ...
  def all_copyloan_funds
    @collateral = CopyCollateral.find_by_id(params[:collateral_id].to_s)
    @loan = CopyLoan.find_by_id(params[:loan_id])
    @funds = @loan.use_funds
    render partial: 'select_copyrelated_funds' 
  end
  ...
  def embed_file_upload
   ...
   def embed_application
    ...
    @contact_required = CustomField.all(:custom_id => "#{@contact_tab.id}", :hide.ne => 1, :required => 1)
    @highlight_required = CustomField.all(:custom_id => "#{@highlight_tab.id}", :hide.ne => 1, :required => 1)
    @transaction_information_required = CustomField.all(:custom_id => "#{@transaction_information_tab.id}", :hide.ne => 1, :required => 1)
    @borrowers_required = CustomField.all(:custom_id => "#{@borrowers_tab.id}", :hide.ne => 1, :required => 1)
    @collateral_required = CustomField.all(:custom_id => "#{@collateral_tab.id}", :hide.ne => 1, :required => 1)
    
    # abort("#{@collateral_required.inspect}")
   end
   ...
   # abort("#{filearr.inspect}")
    ...
    unless check_loan.blank?
      if check_loan.delete_admin == 1
        ...
        Loan.delete_all(:id => check_loan.id.to_i)
        last_loan = Loan.last
        new_id = last_loan.id+1
        @loanInfo = Loan.new
      else
    ...
    loanarray['_LoanSummaryWhatareyoulookingfor'] = "#{objArray['_ExitStrategyHowwillyoupaytheloanoff']}"
    ...
    unless objArray['sndr'].blank?
      @loanInfo.sender_username = "#{objArray['sndr']}"  
    end
    
    unless objArray['sndr'].blank?
      @loanInfo.receiver_email = "#{objArray['rcvr']}"
      @loanInfo.email = "#{objArray['rcvr']}"
    end
    ...
    unless objArray['feature_image'].blank?
      ...
      if img.include? "storage.googleapis.com"
        ...
        unless img.blank?
          ...
          server_host = request.host
          if server_host.include? "idealview"
            img_url = "https://#{objArray['sndr']}.idealsuite.com/#{img}"
          else
            img_url = "http://#{objArray['sndr']}.xlr8seo.com/#{img}"
          end
          img = URI.encode("#{img_url}")
          require 'open-uri'
          File.open(Rails.root.join('public', 'temp', "#{file_name}"), 'wb') do |file|
            uri = URI(img)
        ...
        if key == "folder"
          ...
          loan_folders(new_id, files)
           ...
           value['file_url'].each do |nfile|
              ...
              # if nfile=="https://storage.googleapis.com/idealsuitestorage/Folders%20for%20ravidigi%20loans%2FAccounts%2F2016-03-21%20Loan%20for%20shop%2FAccounting%2F14609810062015%2B-%2B2.jpeg"
              #   abort("here1")
              # end
              #  if nfile=="https://storage.googleapis.com/idealsuitestorage/Folders%20for%20ravidigi%20loans%2FAccounts%2F2016-03-21%20Loan%20for%20shop%2FOther%2Fimagess%2F2016112410451018April2.png"
              #   abort("here2")
              # end
              ...
              # encode_url = URI.encode("#{nfile}")
              url = URI.parse("#{nfile}")
              req = Net::HTTP.new(url.host, url.port)
              req.use_ssl = true
              response= req.request_head url.path
              # abort("#{nfile} -- #{response.code}")
              if response.code.to_i == 200
                if extension==".png" || extension==".jpg" || extension==".jpeg"
                  ...
                  File.open(Rails.root.join('public', 'temp', "#{file_name}"), 'wb') do |file|
                      ...
                      file << open("#{nfile}").read
                       structure = "outsider/#{new_id}/#{file_name}"
                        store_data = StorageBucket.files.new(
                          key: "#{structure}",
                          body: open("#{nfile}").read,
                          public: true
                        );
                       up_size = File.size("#{Rails.root}/public/temp/#{file_name}")
                      ...
                      store_data.save
                      image = Image.new(:loan_id=>new_id.to_i,  :file_id=>structure, :name=>file_name, :url=>store_data.public_url, :from=>"google");
                      image.featured = false
                      image.file_size=@file_mb
                      image.save
                        # end
    ...
    @cfolders = CustomFolder.all(:loan_id => new_id.to_i)
    @cfolders.each do |cflder|
      if cflder.files_by_folder(new_id.to_i).blank?
        CustomFolder.delete_all(:id => "#{cflder.id}")
      end
    end
    
    rsp=Hash.new
    if @loanInfo.save
      progress= check_progress(new_id.to_i)
      rsp['status'] = 1
      rsp['loan_id'] = new_id
    else
      rsp['status'] = 0
  ...
  def cpc_market_loan
      ...
      loanarray['_NetLoanAmountRequested0'] = "#{objArray['_NetLoanAmountRequested0']}"
      loanarray['_ExitStrategyHowwillyoupaytheloanoff'] = "#{objArray['_ExitStrategyHowwillyoupaytheloanoff']}"
      loanarray['interest_rate'] = "#{objArray['loan_interest']}"
      ...
      unless objArray['unearnedFees'].blank?
        loanarray['unearned_fees'] = objArray['unearnedFees']
      end

      unless objArray['borrowersName'].blank?
        loanarray['borrower_name'] = objArray['borrowersName']
      end

      unless objArray['guarantorsName'].blank?
        loanarray['guarantor_name'] = objArray['guarantorsName']
      end

      unless objArray['contact_name'].blank?
        loanarray['FirstName'] = objArray['contact_name']
      end

      unless objArray['contact_email'].blank?
        loanarray['Email'] = objArray['contact_email']
      end

      unless objArray['contact_phone'].blank?
        loanarray['Phone'] = objArray['contact_phone']
      end

      unless objArray['type'].blank?
        loanarray['type'] = objArray['type']
      end
      ...
      if @loanInfo.save
        ...
        check = MarketplaceLoan.find_by_loan_id(@loanInfo.id.to_i)
        if check.blank?
          record = MarketplaceLoan.new
          record.name = "#{objArray['name']}"
          record.loan_id = @loanInfo.id.to_i
          record.created_date = Time.now
          record.created_by = "Idealsuite"
          record.type = params[:type]
          record.info = @infoArray
          record.status = 1
          record.save
        else
          check.name = "#{objArray['name']}"
          check.loan_id = @loanInfo.id.to_i
          check.created_date = Time.now
          check.created_by = "Idealsuite"
          check.type = params[:type]
          check.info = @infoArray
          check.status = 1
          check.save
        end
          ...
          if key == "folder"
            ...
            loan_folders(loan_id, files)
             ...
             value['file_url'].each do |nfile|
                ...
                # newtime= Time.now.strftime("%m-%d-%y_%H-%M-%S")
                # new_file_name = "file#{i}#{newtime}.png"
                # if nfile=="https://storage.googleapis.com/idealsuitestorage/Folders for ravidigi loans%2FAccounts%2F2016-03-21%20Loan%20for%20shop%2FAccounting%2F14609810062015%2B-%2B2.jpeg"
                #   abort("here2")
                # end
                # if nfile=="https://storage.googleapis.com/idealsuitestorage/Folders%20for%20ravidigi%20loans%2FAccounts%2F2016-03-21%20Loan%20for%20shop%2FOther%2Fimagess%2F2016112410451018April2.png"
                #   abort("here2")
                # end
                fileurl = URI.unescape(nfile)
                # abort("File URL -- #{fileurl} URL -- #{nfile}")
                uri = URI("#{nfile}")
                path = uri.path
                pathname = Pathname.new(path)
                extension = pathname.extname

                encode_url = URI.encode("#{fileurl}")
                ...
                if response.code.to_i == 200
                  if extension==".png" || extension==".jpg" || extension==".jpeg"
                    ...
                    File.open(Rails.root.join('public', 'temp', "#{file_name}"), 'wb') do |file|
                        ...
                        file << open("#{encode_url}").read
                         structure = "outsider/#{loan_id}/#{file_name}"
                          store_data = StorageBucket.files.new(
                            key: "#{structure}",
                            body: open("#{encode_url}").read,
                            public: true
                          );
                         up_size = File.size("#{Rails.root}/public/temp/#{file_name}")
                        ...
                        store_data.save
                        image = Image.new(:loan_id=>loan_id.to_i,  :file_id=>structure, :name=>file_name, :url=>store_data.public_url, :from=>"google");
                        image.featured = false
                        image.file_size=@file_mb
                        if image.save
                          done_save = "yes"
                    ...
                    #   image = Image.new(:loan_id=>loan_id.to_i, :name=>file_name, :url=>store_data.public_url, :from=>"google");
                    #   image.featured = true
                    #   image.file_size=@file_mb
                    #   image.save 
                    # else
  ...
  def ids_login
    ...
    server_host = request.host
    if server_host.include? "idealview"
      check_url = "https://idealsuite.com/index.php?r=loginapi/checkdomain"
      uri = URI.parse("#{check_url}");
    ...
    if rsp['status'] == true
      ...
      if server_host.include? "idealview"
        url = "https://#{params[:subdomain]}.idealsuite.com/index.php?r=loginapi"
        uInfo = User.find_by_id("#{current_user.id}")
        uri = URI.parse("#{url}");
        http = Net::HTTP.new(uri.host, uri.port)
        http.use_ssl = true
  ...
  def create_market_place
    ...
    unless loan_ids.blank?
      loan_ids.each do |id|
        ...
        loanInfo = Loan.find_by_id(id.to_i)
        @mloan = MarketplaceLoan.new
        @mloan.name = loanInfo.name
        @mloan.info = loanInfo.info
        @mloan.type = params[:type]
        @mloan.loan_id = id.to_i
        @mloan.created_date = Time.now
        @mloan.created_by = current_user.id
        @mloan.status = 1
        @mloan.hide_fields = loanInfo.hide_fields
        @mloan.save
    ...
    render text: "#{@mloan.id}"
  end

  def loan_market_place
    # abort("#{params.inspect}")
    loan_ids = params[:loans].split(',')
    unless loan_ids.blank?
      loan_ids.each do |id|
  ...
  def market_place
    @market_loans = MarketplaceLoan.all(:status => 2)
    
    # unless @market_loans.blank?
    #   @market_loans.each do |mktloan|
    #     @mloan = MarketplaceLoan.new
    #     @mloan.name = mktloan.loan.name
    #     @mloan.info = mktloan.loan.info
    #     @mloan.type = mktloan.type
    #     @mloan.user_id = mktloan.user_id
    #     @mloan.loan_id = mktloan.loan_id
    #     @mloan.created_date = mktloan.created_date
    #     @mloan.access_id = mktloan.access_id
    #     @mloan.save
    #   end
    # end
  ...
  def market_access
    mInfo = MarketplaceLoan.find_by_id("#{params[:market_id]}")

    @find = MarketAccess.find_by_market_id("#{params[:market_id]}")
    if @find.blank?
      access = MarketAccess.new
      access.loan_id = mInfo.loan_id.to_i
      access.user_id = "#{current_user.id}"
      access.market_id = "#{params[:market_id]}"
      access.save
      
      mInfo.access_id = access.id
      mInfo.save
    end
    render text:"done"
  end
  ...
  def more_details
    @loan = MarketplaceLoan.find_by_id(params[:loan_id])
    render partial: 'more_details'
  end

  def search_market_loan
    # abort("#{params.inspect}")
    @loans = MarketplaceLoan.all(:status => 2)
    # abort("#{@loans.inspect}")
    unless @loans.blank?
      ...
      @loans.each do |mloan|
       # abort("Min--#{min} Max--#{max}")
        ...
        if min==0 && max!=0
          if mloan.info['_NetLoanAmountRequested0'].to_i >= min.to_i && mloan.info['_NetLoanAmountRequested0'].to_i <= max.to_i
            loanamount << mloan.id
          end
        end
        ...
        if max==0 && min!=0
          if mloan.info['_NetLoanAmountRequested0'].to_i >= min.to_i
            loanamount << mloan.id
          end
        end
        ...
        unless params[:loan_range].blank?
          ...
          if loan_range.length == 2
            if mloan.info['_NetLoanAmountRequested0'].to_i >= loan_range[0].to_i && mloan.info['_NetLoanAmountRequested0'].to_i <= loan_range[1].to_i
              loanamount << mloan.id
            end
          else
            if mloan.info['_NetLoanAmountRequested0'].to_i >= 2000000
              loanamount << mloan.id
            end 
          end
        end
        ...
        unless params[:term].blank?
          if params[:term] == mloan.info['_DesiredTermLength']
            loanterm << mloan.id
          end
        end
        ...
        unless params[:interest].blank?
          if params[:interest].to_i == 0
            if mloan.info['interest_rate'].to_i >= 0 && mloan.info['interest_rate'].to_i <= 7
              loaninterest << mloan.id
            end
          elsif params[:interest].to_i == 7
            if mloan.info['interest_rate'].to_i >= 7 && mloan.info['interest_rate'].to_i <= 12
              loaninterest << mloan.id
            end
          elsif params[:interest].to_i == 12
            if mloan.info['interest_rate'].to_i >= 12
              loaninterest << mloan.id
            end
          end
        end
        ...
        unless params[:state].blank?
          if mloan.info['State3'] == params[:state] 
            loanstate << mloan.id
          end
        end
      ...
      if params[:min_range].blank? && params[:max_range].blank? && params[:term].blank? && params[:interest].blank? && params[:state].blank?
        ...
        @market_loans = MarketplaceLoan.all(:id => result)
      end
  ...
  def connect_dropbox
    ...
    authorize_url = "https://www.dropbox.com/oauth2/authorize?client_id=ls1qvq2ynybwv19&response_type=code"
  ...
  def check_dropbox_code
    ...
    request = Net::HTTP::Post.new(uri.request_uri)
    # request.set_form_data({"api_key" => "aae5b42c3f0997b8c6c5596b9c1856d89fd25ebe"})
    request.set_form_data({"code" => "#{params[:dropbox_code]}", "grant_type" => "authorization_code", "client_id"=>"ls1qvq2ynybwv19", "client_secret" => "lqkgklyc6ttuq5e"})
    response = http.request(request)
    jresult = response.body
  ...
  def idv_market_loans
    all_loans = MarketplaceLoan.paginate(:per_page => params[:records_per_page], :page => params[:page_num], :order => :id.desc, :status => 2)
    unless all_loans.blank?
      loanInfo = Array.new
      all_loans.each do |aloan|
         myHash = Hash.new
         image = aloan.loanInfo.featured_image
         myHash['loan_id'] = aloan.loan_id
         
         
        unless aloan.type.blank? 
          myHash['type'] = aloan.type
        else
          myHash['type'] = "Loan Offer"
        end
        
        if aloan.info['_LendingCategory'] == "Private Real Estate Loan"
          @asset_type = aloan.info['_LendingTypes'] 
        elsif aloan.info['_LendingCategory'] == "Business Financing"
          @asset_type = aloan.info['_BusinessFinancingTypes']
        elsif aloan.info['_LendingCategory'] == "Equity and Crowdfunding"
          @asset_type = aloan.info['_EquityandCrowdFunding']
        elsif aloan.info['_LendingCategory'] == "Residential or Commercial Mortgage"
          @asset_type = aloan.info['_MortageTypes'] 
        else
          @asset_type =""
        end 
        

        if aloan.info['_DesiredTermLength'] == "3" 
          @desired =  "Less Than 3 Months" 
        elsif aloan.info['_DesiredTermLength'] == "6"
          @desired = "3 to 6 Months"
         elsif aloan.info['_DesiredTermLength'] == "12"
           @desired = "6 to 12 Months"
         elsif aloan.info['_DesiredTermLength'] == "24"
          @desired = "12 to 24 Months"
         elsif aloan.info['_DesiredTermLength'] == "25" 
       ...
       begin 
        unless aloan.name.blank?
          myHash['name']  = aloan.name
        else
          myHash['name']  = ""
        end
        if defined? aloan.info['_NetLoanAmountRequested0']
            myHash['amount'] = aloan.info['_NetLoanAmountRequested0']
        else
          myHash['amount'] = ""
        end

        if defined? aloan.info['_LendingCategory']
            myHash['loan_category'] = aloan.info['_LendingCategory']
        else
          myHash['loan_category'] = ""
        end

        if defined? aloan.info['address3']
            myHash['address'] = aloan.info['address3']
        else
          myHash['address'] = ""
        end

        if defined? aloan.info['State3']
            myHash['state'] = aloan.info['State3']
        else
            myHash['state'] = ""
        end

         if defined? aloan.info['City3']
            myHash['city'] = aloan.info['City3']
        else
            myHash['city'] = ""
        end

        if defined? aloan.info['interest_rate']
            myHash['interest'] = aloan.info['interest_rate']
        else
            myHash['interest'] = ""
        end
  ...
  def num_market_loans
    num_count = MarketPlace.count
    render text:"#{num_count}"
  end
  ...
  def create_report_form
    @borrower = Borrower.find_by_id("#{params[:borrower]}")

    # @credit_info = CreditInfo.find_by_borrower_id
    render partial:"report_form"
  end

  def check_report
    @credit = CreditReport.find_by_borrower_id("#{params[:borrower]}")

    # @credit_info = CreditInfo.find_by_borrower_id
    render partial:"credit_report"
  end

   def report_borrowers
    @loan = Loan.find_by_id(params[:loan_id].to_i)
    @borrowers = Borrower.all(:loan_id => params[:loan_id].to_i, :order => :id.desc)
    # abort("#{@borrowers.inspect}")
  ...
  def save_credit_credential
    @info = CreditKey.new
    @info.user_id = "#{current_user.id}"
    @info.username = "#{params[:uname]}"
  ...
  def save_credit_info
    ...
    @credit_info = CreditInfo.find_by_borrower_id("#{params[:borrower_id]}")
    if @credit_info.blank?
      @credit_info = CreditInfo.new
    end

    @credit_info.firstname = "#{params[:firstname]}"
    @credit_info.middlename = "#{params[:middlename]}"
    @credit_info.lastname = "#{params[:lastname]}"
    @credit_info.borrower_id = "#{params[:borrower_id]}"
    @credit_info.ein = "#{params[:ein]}"
    @credit_info.address = "#{params[:card_address]}"
    @credit_info.city = "#{params[:card_city]}"
    @credit_info.state = "#{params[:card_state]}"
    @credit_info.postal_code = "#{params[:card_postal]}"
    @credit_info.payment_form_status = "#{params[:payment_form_status]}"
   
    if params[:payment_form_status] == "1" &&  @empty != 1
       @request_time = Time.now.strftime("%Y-%m-%dT%H:%M:%S")
       # abort("#{@request_time}")
       ...
       # abort("#{@card_num}")
        ...
        xmlval = '<REQUEST_GROUP MISMOVersionID="2.3.1">
                    ...
                    <REQUEST RequestDatetime="'+@request_time+'" InternalAccountIdentifier="" LoginAccountIdentifier="'+params[:username]+'" LoginAccountPassword="'+params[:password]+'">
                    <REQUEST_DATA>
                    <CREDIT_REQUEST MISMOVersionID="2.3.1" LenderCaseIdentifier="lender_'+params[:borrower_id]+'" RequestingPartyRequestedByName="idvServicer">
                    <CREDIT_REQUEST_DATA CreditRequestID="idv_request_'+@credit_info.id+'" BorrowerID="idv_b_'+params[:borrower_id]+'" CreditReportIdentifier="" CreditReportRequestActionType="Submit" CreditReportType="Merge" CreditRequestDateTime="'+@request_time+'" CreditRequestType="Individual">
                              <CREDIT_REPOSITORY_INCLUDED _EquifaxIndicator="Y" _ExperianIndicator="Y" _TransUnionIndicator="Y"></CREDIT_REPOSITORY_INCLUDED>
                            </CREDIT_REQUEST_DATA>
                            <SERVICE_PAYMENT _AccountIdentifier="'+@card_num+'" _AccountHolderName="'+params[:card_name]+'" _AccountHolderStreetAddress="'+params[:card_address]+'" _AccountHolderCity="'+params[:card_city]+'" _AccountHolderState="'+params[:card_state]+'" _AccountHolderPostalCode="'+params[:card_postal]+'" _AccountExpirationDate="'+params[:card_expiry]+'" _SecondaryAccountIdentifier="'+params[:card_cvv]+'">
                             </SERVICE_PAYMENT>
                            <LOAN_APPLICATION>
                              <BORROWER BorrowerID="idv_b_'+params[:borrower_id]+'" _FirstName="Wayne" _MiddleName="" _LastName="Nehwadowich" _NameSuffix="" _AgeAtApplicationYears="" _PrintPositionType="Borrower" _SSN="000000001" MaritalStatusType="NotProvided">
                                <_RESIDENCE _StreetAddress="'+params[:card_address]+'" _City="'+params[:card_city]+'" _State="'+params[:card_state]+'" _PostalCode="'+params[:card_postal]+'" BorrowerResidencyType="Current"></_RESIDENCE>
                              </BORROWER>
                            </LOAN_APPLICATION>
                          </CREDIT_REQUEST>
                        </REQUEST_DATA>
        ...
        if status == "READY"
          @html_resp = result["RESPONSE_GROUP"]["RESPONSE"]["RESPONSE_DATA"]["CREDIT_RESPONSE"]["EMBEDDED_FILE"]["DOCUMENT"]
          @credit_info.save
          @check_exist = CreditReport.find_by_credit_info_id("#{@credit_info.id}")
          if @check_exist.blank?
            @check_exist = CreditReport.new
            @check_exist.credit_info_id = "#{@credit_info.id}"
            @check_exist.borrower_id = "#{params[:borrower_id]}"
          end
          @check_exist.report = @html_resp
          @check_exist.save
          @render_text = "done"
          @done=1

        else
          # @html_resp = result["RESPONSE_GROUP"]["RESPONSE"]["RESPONSE_DATA"]["CREDIT_RESPONSE"]["CREDIT_ERROR_MESSAGE"]
          # # abort("#{@html_resp.inspect}")
          # unless @html_resp.blank?
          #   @render_text = "#{@html_resp['_Text']}"
          #   if @render_text == "Login Failed"
          #     @render_text = "Login Failed, Invalid username or password."
          #   end
          # else
            @render_text = "error"
          # end
  ...
  ########################## Previous Functionality ##############################
    ...
    def do_not_sync
      ...
      dropbox = DropboxToken.find_by_id("#{current_user.dropbox.id}")
      dropbox.not_sync = 1
      dropbox.save
      # abort("#{uInfo.inspect}")
      render text: "done"
  ...
  ########################## End Previous Functionality ##############################

  def do_not_sync_loans
    
    loan_ids = params[:loan_ids].split(',')
    loan_ids.each do |id|
    @loan_change = Loan.find_by_id(id.to_i)

      unless @loan_change.blank?
        @loan_change.not_sync = 1
        @loan_change.save
      end
       # abort("#{@loan_change.inspect}")
    end
  ...
  def send_to_approval
    loan_id = "#{params[:loan_id]}"
    @mloan = MarketplaceLoan.find_by_id(loan_id)
    @mloan.status = 1
    @mloan.save


    LoanUrlMailer.send_marketplace("#{params[:loan_id]}").deliver
  ...
  def market_loan_detail
      # abort("Here")
    @loan = MarketplaceLoan.find_by_id("#{params[:id]}")
    @borrowers = Borrower.all(:loan_id => @loan.loan_id, :hide.ne=>1)
    @collaterals = Collateral.all(:loan_id => @loan.loan_id, :hide.ne=>1, :order => :sort_id.asc)
    @files = FolderFile.all(:loan_id => @loan.loan_id)

    #################### Analysis ############################
    col_acres = CollateralAcres.all(:loan_id => @loan.loan_id.to_i)
    @colacres = Hash.new
    col_acres.each do |col_acre|
      @colacres[col_acre.collateral_id.to_s] = col_acre.value
    end
    col_sqs = CollateralSqfootage.all(:loan_id => @loan.loan_id.to_i) 
    @colsqs = Hash.new
    col_sqs.each do |col_sq|
      @colsqs[col_sq.collateral_id.to_s] = col_sq.value
    end
    # abort("#{@colsqs.inspect}")
    col_nois = CollateralNoi.all(:loan_id => @loan.loan_id.to_i)
    @colnois = Hash.new
    col_nois.each do |col_noi|
      @colnois[col_noi.collateral_id.to_s] = col_noi.value
    end
    col_gross = CollateralGross.all(:loan_id => @loan.loan_id.to_i)
    @colgross = Hash.new
    col_gross.each do |col_gros|
      @colgross[col_gros.collateral_id.to_s] = col_gros.value
    end
  ...
  def marketplace_approval
    if current_user.blank?
      redirect_to "/users/sign_in"
    else
      @total = MarketplaceLoan.count(:status.ne => 0)
      @loans = MarketplaceLoan.paginate(:status.ne => 0, :order => :id.desc, :per_page => 10, :total_entries => @total)
      @page = 1
    end
  end

  def approved_marketplace
    @mloan = MarketplaceLoan.find_by_id("#{params[:loan_id]}")
    @mloan.status = 2
    @mloan.save
  ...
  def shop_paging
    @total = MarketplaceLoan.count(:status.ne => 0)
    @loans = MarketplaceLoan.paginate(:status.ne => 0, :order => :id.desc, :per_page => 10, :page => params[:page], :total_entries => @total)
    unless params[:records].blank?
  ...
  def loan_sync
    ...
    @loan_change = Loan.find_by_id(params[:loan_id].to_i)

      unless @loan_change.blank?
        if params[:sync] == "on"
          @loan_change.not_sync = 0
        elsif params[:sync] == "off"
           @loan_change.not_sync = 1
        end
  ...
  def check_progress loan_id
    @loan_rec = Loan.find_by_id(loan_id.to_i)
    @filled = 0
    unless  @loan_rec.name.blank?
      @filled = @filled+1
    end
    @check_fields = Array.new
    unless @loan_rec.info.blank?
      if defined? @loan_rec.info['_LoanSummaryWhatareyoulookingfor']
        unless @loan_rec.info['_LoanSummaryWhatareyoulookingfor'].blank?
          @filled = @filled+1
      ...
      if defined? @loan_rec.info['transaction_type']
        unless @loan_rec.info['transaction_type'].blank?
          @filled = @filled+1
          @check_fields << "transaction_type"
        end
      end

      if defined? @loan_rec.info['isBroker']
        unless @loan_rec.info['isBroker'].blank?
          @filled = @filled+1
          @check_fields << "isBroker"
        end
      end

      if defined? @loan_rec.info['free_clear']
        unless @loan_rec.info['free_clear'].blank?
          @filled = @filled+1
          @check_fields << "free_clear"
        end
      end

      if defined? @loan_rec.info['_NetLoanAmountRequested0']
        unless @loan_rec.info['_NetLoanAmountRequested0'].blank?
          @filled = @filled+1
          @check_fields << "_NetLoanAmountRequested0"
        end
      end

      if defined? @loan_rec.info['_EstimatedMarketValue']
        unless @loan_rec.info['_EstimatedMarketValue'].blank?
          @filled = @filled+1
          @check_fields << "_EstimatedMarketValue"
        end
      end

      if defined? @loan_rec.info['_DesiredTermLength']
        unless @loan_rec.info['_DesiredTermLength'].blank?
          @filled = @filled+1
           @check_fields << "_DesiredTermLength"
        end
      end

      if defined? @loan_rec.info['Address3']
        unless @loan_rec.info['Address3'].blank?
          @filled = @filled+1
          @check_fields << "Address3"
        end
      end

      if defined? @loan_rec.info['City3']
        unless @loan_rec.info['City3'].blank?
          @filled = @filled+1
          @check_fields << "City3"
        end
      end

      if defined? @loan_rec.info['State3']
        unless @loan_rec.info['State3'].blank?
          @filled = @filled+1
          @check_fields << "State3"
        end
      end

      if defined? @loan_rec.info['_ExpectedCloseDate']
        unless @loan_rec.info['_ExpectedCloseDate'].blank?
          @filled = @filled+1
          @check_fields << "_ExpectedCloseDate"
        end
      end

      if defined? @loan_rec.info['_LendingCategory']
        unless @loan_rec.info['_LendingCategory'].blank?
          @filled = @filled+1
          @check_fields << "_LendingCategory"
        end
      end

      if defined? @loan_rec.lending_type
        unless @loan_rec.lending_type.blank?
          @filled = @filled+1
          @check_fields << "_LendingTypes"
        end
      end

      if defined? @loan_rec.info['_ExitStrategyHowwillyoupaytheloanoff']
        unless @loan_rec.info['_ExitStrategyHowwillyoupaytheloanoff'].blank?
          @filled = @filled+1
          @check_fields << "_ExitStrategyHowwillyoupaytheloanoff"
        end
      end

      if defined? @loan_rec.info['FirstName']
        unless @loan_rec.info['FirstName'].blank?
          @filled = @filled+1
          @check_fields << "FirstName"
        end
      end

      if defined? @loan_rec.info['Email']
        unless @loan_rec.info['Email'].blank?
          @filled = @filled +1
          @check_fields << "Email"
        end
      end
    ...
    unless @loan_rec.borrower.blank?
      @check_fields << "borrowers"
      @filled=@filled+1
    end

    unless @loan_rec.collateral.blank?
      @check_fields << "collateral"
      @filled  = @filled+1
    end

    unless @loan_rec.use_of_fund.blank?
       @check_fields << "use_of_fund"
      @filled = @filled+1
    end

    

    unless @loan_rec.images.blank?
      @check_fields << "images"
      @filled = @filled+1
    end

    unless @loan_rec.documents.blank?
      @check_fields << "documents"
      @filled = @filled+1
    else
      unless @loan_rec.other_files.blank?
         @check_fields << "other_files"
        @filled = @filled+1
      end
    end
    

    @field_num = @filled.to_i
    # abort("#{@check_fields.inspect} -- #{@filled}")
    lInfo = Loan.find_by_id(loan_id.to_i)
    lInfo.progress = @field_num.to_i
    lInfo.save

    return @field_num
    # total = 23 
    # pre_result = (@field_num.to_f/total.to_f)
    # result = pre_result * 100
    # display_result = result.round(0)
  end

  def genrate_loan_pdf
    @loan = Loan.find_by_id(params[:id].to_i)
  end

  def loan_generate_pdf
     if params[:loan]!=""
      filename=params[:loan]+Time.now.strftime("%m%d%y%H%M%S")
    else
    ...
    pdf_link = "#{request.base_url}/loans/show_pdf/#{pdfname}"

    if !params[:email].blank?
      LoanUrlMailer.send_loan_summary("#{params[:loan]}", pdf_link, "#{params[:email]}").deliver
    end
    render plain: "#{pdfname}"
  end
